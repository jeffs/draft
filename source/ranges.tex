...
\indexlibraryglobal{ranges::ssize}%

\pnum
The name \tcode{ssize} denotes
a customization point object\iref{customization.point.object}.
The expression \tcode{ranges::ssize(E)}
...
\begin{codeblock}
static_cast<ptrdiff_t>(ranges::size(E)); // \ref{range.prim.size}
\end{codeblock}

\item
Otherwise:
\begin{codeblock}
static_cast<range_difference_t<T>>(ranges::size(E)); // \ref{range.prim.size}
\end{codeblock}
\end{itemize}
