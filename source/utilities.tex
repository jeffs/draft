...
\pnum
\constraints
\tcode{is_copy_constructible_v<first_type>} is \tcode{true} and
...
\pnum
\mandates
\tcode{is_constructible_v<first_type, Args1...>} is \tcode{true} and
\tcode{is_constructible_v<second_type, Args2...>} is \tcode{true}.
...
\pnum
\constraints
\tcode{is_assignable_v<first_type\&, const U1\&>} is \tcode{true} and
\tcode{is_assignable_v<second_type\&, const U2\&>} is \tcode{true}.
...
\pnum
\constraints
\tcode{is_move_assignable_v<first_type>} is \tcode{true} and
\tcode{is_move_assignable_v<second_type>} is \tcode{true}.
...
\pnum
\constraints
\tcode{is_assignable_v<first_type\&, U1>} is \tcode{true} and
\tcode{is_assignable_v<second_type\&, U2>} is \tcode{true}.
...
\pnum
\returns
If \tcode{I} is 0, returns a reference to \tcode{p.first};
if \tcode{I} is 1, returns a reference to \tcode{p.second}.
\end{itemdescr}
...
\pnum
\constraints
\tcode{is_assignable_v<$\tcode{T}_i$\&, $\tcode{U}_i$>} is \tcode{true} for all $i$ and
...
\pnum
\constraints
\tcode{sizeof...(Types)} is 2 and
\tcode{is_assignable_v<$\tcode{T}_0$\&, const U1\&>} is \tcode{true} and
\tcode{is_assignable_v<$\tcode{T}_1$\&, const U2\&>} is \tcode{true}.
...
\constraints
\tcode{sizeof...(Types)} is 2 and
\tcode{is_assignable_v<$\tcode{T}_0$\&, U1>} is \tcode{true} and
\tcode{is_assignable_v<$\tcode{T}_1$\&, U2>} is \tcode{true}.
...
meet the \oldconcept{UnaryTypeTrait} requirements\iref{meta.rqmts}
...
each of the three templates meet the \oldconcept{TransformationTrait}
...
\pnum
\constraints
There is exactly one occurrence of \tcode{T} in \tcode{Types...} and
\tcode{is_constructible_v<T, Args...>} is \tcode{true}.
...
\pnum
\mandates
\tcode{unique_ptr<T1, D1>::pointer} is implicitly convertible to \tcode{CT} and
\tcode{unique_ptr<T2, D2>::pointer} is implicitly convertible to \tcode{CT}.
...
\pnum
\remarks
If \tcode{decltype(\&F::operator())} is of the form
...
\pnum
\expects
For any two values \tcode{A} and \tcode{B}
of the type \tcode{iterator_traits<RandomAccessIterator1>::val\-ue_type},
...
\tcode{template<class T>}\br
 ...
 \mandates \tcode{T} is a (possibly cv-qualified)
